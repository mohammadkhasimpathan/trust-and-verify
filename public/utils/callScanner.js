...
function analyzePhoneNumber(numberStr) {
  ...
  if (CALLED_BLACKLIST[cleanNumber]) {
    ...
    indicators.push({
      id: `CALL_BLACKLIST_${cleanNumber}`,
      category: 'REPUTATION',
      severity: 'CRITICAL',
      weight: 100,
      title: `Blacklist Match: ${entry.name}`,
      description: `Verified threat matching phone records database. ${entry.description}`,
      evidence: `${entry.reports} reports logged for ${cleanNumber}.`,
      recommendation: 'Block this number immediately. Do not answer calls or reply to texts from it.',
    ...
    if (cleanNumber.length < 7) {
      ...
      indicators.push({
        id: 'CALL_MALFORMED_NUMBER',
        category: 'ROUTING',
        severity: 'LOW',
        weight: 25,
        title: 'Malformed Number Structure',
        description: 'The phone number has too few digits, indicating it is likely generated by custom VOIP spoofing software.',
        evidence: `Length: ${cleanNumber.length} digits`,
        recommendation: 'Exercise caution if answering. Do not provide personal information.',
        source: 'LOCAL_HEURISTIC'
      });
    }
    ...
    if (prefixMatched) {
      ...
      indicators.push({
        id: 'CALL_TOLL_FREE_VOIP',
        category: 'ROUTING',
        severity: 'MEDIUM',
        weight: 35,
        title: 'Toll-Free VOIP Gateway',
        description: 'The number originates from a toll-free VoIP prefix. VoIP gateways are easily rented online by scammers to obfuscate geographic location.',
        evidence: `Prefix: ${prefixMatched}`,
        recommendation: 'Verify the caller\'s identity through an official support channel.',
        source: 'LOCAL_HEURISTIC'
      });
    }
